...
exports.register = (req, res) => {
  ...
  if (!aluno.nome || !aluno.senha || !aluno.cep || !aluno.dataNascimento || !aluno.cpf) {
    return res.status(400).json({ error: 'Preencha todos os campos obrigatórios!' });
  }

  AlunoModel.create(aluno, (err, id) => {
    if (err) {
      return res.status(500).json({ error: 'Erro ao cadastrar aluno.' });
    }
    res.status(201).json({ message: 'Aluno cadastrado com sucesso!', id });
  });
};
